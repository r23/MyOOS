				...
				<table class="table table-striped w-100">
					<thead>
						<tr>
							...
							<td align="center"><?php echo TABLE_HEADING_FILE_DATE; ?></td>
							<td align="right"><?php echo TABLE_HEADING_FILE_SIZE; ?></td>
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($buInfo) && is_object($buInfo) && ($entry == $buInfo->file) ) { echo '<button class="btn btn-info" type="button"><i class="fa fa-check"></i></button>'; } else { echo '<a href="' . oos_href_link_admin($aContents['export_excel'], 'file=' . $entry) . '">' . oos_image(OOS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
    $dir->close();
...
<?php
  ...
  switch ($action) {
    ...
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . oos_submit_button('delete', BUTTON_DELETE) . ' <a href="' . oos_href_link_admin($aContents['export_excel'], 'file=' . $buInfo->file) . '">' . oos_button('cancel', BUTTON_CANCEL) . '</a>');
      break;
