<?php
/* ----------------------------------------------------------------------
   $Id: affiliate_contact.php,v 1.1 2007/06/08 17:14:40 r23 Exp $
   ...
   File: affiliate_contact.php,v 1.5 2003/02/17 20:11:59 thomasamoulton  
   ----------------------------------------------------------------------
   OSC-Affiliate
  ...
  if ( ($action == 'send_email_to_user') && isset($_POST['affiliate_email_address']) && !isset($_POST['back_x']) ) {
    switch ($_POST['affiliate_email_address']) {
      case '***':
        $mail_result = $dbconn->Execute("SELECT affiliate_firstname, affiliate_lastname, affiliate_email_address FROM " . $oostable['affiliate_affiliate'] . " ");
        $mail_sent_to = TEXT_ALL_AFFILIATES;
        break;

      default:
        $affiliate_email_address = oos_db_prepare_input($_POST['affiliate_email_address']);

        $mail_result = $dbconn->Execute("SELECT affiliate_firstname, affiliate_lastname, affiliate_email_address FROM " . $oostable['affiliate_affiliate'] . " WHERE affiliate_email_address = '" . oos_db_input($affiliate_email_address) . "'");
    ...
    while ($mail = $mail_result->fields) {
      ...
      if (EMAIL_USE_HTML == 'true') {
        ...
        $send_mail->AltBody = $text;
      } else {
        $send_mail->Body = $text;
      }
      $send_mail->AddAddress($mail['affiliate_email_address'], $mail['affiliate_firstname'] . ' ' . $mail['affiliate_lastname']);
      $send_mail->Send();
    ...
    oos_redirect_admin(oos_href_link_admin($aFilename['affiliate_contact'], 'mail_sent_to=' . urlencode($mail_sent_to)));
  }
...
?>
          <tr><?php echo oos_draw_form('mail', $aFilename['affiliate_contact'], 'action=send_email_to_user'); ?>
...
?>
                <table border="0" width="100%" cellpadding="0" cellspacing="2">
                  <tr>
                    ...
                    <td align="right"><?php echo '<a href="' . oos_href_link_admin($aFilename['affiliate_contact']) . '">' . oos_image_swap_button('cancel','cancel_off.gif', IMAGE_CANCEL) . '</a> ' . oos_image_swap_submits('send_mail','send_mail_off.gif', IMAGE_SEND_EMAIL); ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
...
?>
          <tr><?php echo oos_draw_form('mail', $aFilename['affiliate_contact'], 'action=preview'); ?>
...
<?php
    ...
    $mail_result = $dbconn->Execute("SELECT affiliate_email_address, affiliate_firstname, affiliate_lastname FROM " . $oostable['affiliate_affiliate'] . " ORDER BY affiliate_lastname");
    while($affiliate_values = $mail_result->fields) {
...
?>
              <tr>
                ...
                <td><?php echo oos_draw_pull_down_menu('affiliate_email_address', $affiliate, $_GET['affiliate']);?></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo oos_draw_separator('trans.gif', '1', '10'); ?></td>
              </tr>
              <tr>
                ...
                <td><?php echo oos_draw_input_field('from_name', AFFILIATE_OWNER); ?></td>
              </tr>
              <tr>
                <td class="main"><?php echo TEXT_FROM_MAIL; ?></td>
                <td><?php echo oos_draw_input_field('from_mail',AFFILIATE_EMAIL_ADDRESS); ?></td></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo oos_draw_separator('trans.gif', '1', '10'); ?></td>
              </tr>
              <tr>
                ...
                <td><?php echo oos_draw_input_field('subject', '', 'size="60"'); ?></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo oos_draw_separator('trans.gif', '1', '10'); ?></td>
              </tr>
