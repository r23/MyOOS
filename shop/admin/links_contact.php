<?php
/* ----------------------------------------------------------------------
   $Id: links_contact.php,v 1.1 2007/06/08 17:14:41 r23 Exp $
   ...
   File: links_contact.php,v 1.00 2003/10/03 
   ----------------------------------------------------------------------
   Links Manager
  ...
  if ( ($action == 'send_email_to_user') && isset($_POST['link_partners_email_address']) && !isset($_POST['back_x']) ) {
    switch ($_POST['link_partners_email_address']) {
      case '***':
        $mail_result = $dbconn->Execute("SELECT DISTINCT links_contact_name, links_contact_email FROM " . $oostable['links']);
        $mail_sent_to = TEXT_ALL_LINK_PARTNERS;
        break;

      default:
        $link_partners_email_address = oos_db_prepare_input($_POST['link_partners_email_address']);

        $mail_result = $dbconn->Execute("SELECT links_contact_email, links_contact_name FROM " . $oostable['links'] . " WHERE links_contact_email = '" . oos_db_input($link_partners_email_address) . "'");
    ...
    while ($mail = $mail_result->fields) {
      ...
      $send_mail->AddAddress($mail['links_contact_email'], $mail['links_contact_email']);
      $send_mail->Send();
    ...
    oos_redirect_admin(oos_href_link_admin($aFilename['links_contact'], 'mail_sent_to=' . urlencode($mail_sent_to)));
  }
...
?>
          <tr><?php echo oos_draw_form('mail', $aFilename['links_contact'], 'action=send_email_to_user'); ?>
...
?>
                <table border="0" width="100%" cellpadding="0" cellspacing="2">
                  <tr>
                    ...
                    <td align="right"><?php echo '<a href="' . oos_href_link_admin($aFilename['links_contact']) . '">' . oos_image_swap_button('cancel','cancel_off.gif', IMAGE_CANCEL) . '</a> ' . oos_image_swap_submits('send_mail','send_mail_off.gif', IMAGE_SEND_EMAIL); ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
...
?>
          <tr><?php echo oos_draw_form('mail', $aFilename['links_contact'], 'action=preview'); ?>
...
<?php
    ...
    $mail_result = $dbconn->Execute("SELECT DISTINCT links_contact_email, links_contact_name FROM " . $oostable['links'] . " ORDER BY links_contact_name");
    while($link_partners_values = $mail_result->fields) {
...
?>
              <tr>
                ...
                <td><?php echo oos_draw_pull_down_menu('link_partners_email_address', $link_partners, (isset($_GET['link_partner']) ? $_GET['link_partner'] : ''));?></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo oos_draw_separator('trans.gif', '1', '10'); ?></td>
              </tr>
