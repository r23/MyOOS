<?php
/* ----------------------------------------------------------------------
   $Id: products_expected.php 437 2013-06-22 15:33:30Z r23 $
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id) ) { echo oos_image(OOS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . oos_href_link_admin($aFilename['products_expected'], 'page=' . $_GET['page'] . '&pID=' . $products['products_id']) . '">' . oos_image(OOS_IMAGES . 'icon_information.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
...
<?php
  ...
  if (isset($pInfo) && is_object($pInfo)) {
    ...
    $contents[] = array('align' => 'center', 'text' => '<a href="' . oos_href_link_admin($aFilename['categories'], 'pID=' . $pInfo->products_id . '&cPath=' . $cPath . '&action=new_product') . '">' . oos_image_swap_button('edit','edit_off.gif', IMAGE_EDIT) . '</a>');
