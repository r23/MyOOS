<?php
/* ----------------------------------------------------------------------
   $Id: content_block.php 437 2013-06-22 15:33:30Z r23 $
  ...
  if (!empty($action)) {
    switch ($action) {
      case 'setflag':
        ...
        oos_redirect_admin(oos_href_link_admin($aFilename['content_block'], 'page=' . intval($_GET['page']) . '&bID=' . intval($_GET['bID'])));
        break;
...
<table border="0" width="100%" cellspacing="2" cellpadding="2">
  <tr>
    <td width="<?php echo block_WIDTH; ?>" valign="top"><table border="0" width="<?php echo block_WIDTH; ?>" cellspacing="1" cellpadding="1" class="columnLeft">
...
<?php
  ...
  while ($block = $block_content_result->fields) {
    ...
    if (isset($bInfo) && is_object($bInfo) && ($block['block_id'] == $bInfo->block_id) ) {
      echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['content_block'], 'page=' . $_GET['page'] . '&bID=' . $block['block_id'] . '&action=edit') . '\'">' . "\n";
    } else {
      echo '              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['content_block'], 'page=' . $_GET['page'] . '&bID=' . $block['block_id']) . '\'">' . "\n";
    }
?>
...
<?php
  if ($block['block_status'] == '1') {
    echo '<a href="' . oos_href_link_admin($aFilename['content_block'], 'action=setflag&flag=0&bID=' . $block['block_id'] . '&page=' . $_GET['page']) . '">' . oos_image(OOS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
  } else {
    echo '<a href="' . oos_href_link_admin($aFilename['content_block'], 'action=setflag&flag=1&bID=' . $block['block_id'] . '&page=' . $_GET['page']) . '">' . oos_image(OOS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>';
  }
?></td>
                <td class="dataTableContent" align="center">
<?php
  if ($block['block_login_flag'] == '1') {
    echo '<a href="' . oos_href_link_admin($aFilename['content_block'], 'action=setloginflag&login_flag=0&bID=' . $block['block_id'] . '&page=' . $_GET['page']) . '">' . oos_image(OOS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
  } else {
    echo '<a href="' . oos_href_link_admin($aFilename['content_block'], 'action=setloginflag&login_flag=1&bID=' . $block['block_id'] . '&page=' . $_GET['page']) . '">' . oos_image(OOS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>';
  }
?></td>
                <td class="dataTableContent" align="right"><?php if (isset($bInfo) && is_object($bInfo) && ($block['block_id'] == $bInfo->block_id) ) { echo oos_image(OOS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . oos_href_link_admin($aFilename['content_block'], 'page=' . $_GET['page'] . '&bID=' . $block['block_id']) . '">' . oos_image(OOS_IMAGES . 'icon_information.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
...
?>
              <tr>
                <td align="right" colspan="6" class="smallText"><?php echo '<a href="' . oos_href_link_admin($aFilename['content_block'], 'page=' . $_GET['page'] . '&bID=' . $bInfo->block_id . '&action=new') . '">' . oos_image_swap_button('insert','insert_off.gif', IMAGE_INSERT) . '</a>'; ?></td>
              </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents = array('form' => oos_draw_form('block', $aFilename['content_block'], 'action=insert', 'post', 'enctype="multipart/form-data"'));
      $contents[] = array('text' => TEXT_NEW_INTRO);

      $block_inputs_string = '';
      $languages = oos_get_languages();
      for ($i = 0, $n = count($languages); $i < $n; $i++) {
        $block_inputs_string .= '<br />' . oos_image(OOS_IMAGES . 'flags/' . $languages[$i]['iso_639_2'] . '.gif', $languages[$i]['name']) . '&nbsp;' . oos_draw_input_field('block_name[' . $languages[$i]['id'] . ']');
      }
    ...
    case 'edit':
      ...
      $languages = oos_get_languages();
      for ($i = 0, $n = count($languages); $i < $n; $i++) {
        $block_inputs_string .= '<br />' . oos_image(OOS_IMAGES . 'flags/' . $languages[$i]['iso_639_2'] . '.gif', $languages[$i]['name']) . '&nbsp;' . oos_draw_input_field('block_name[' . $languages[$i]['id'] . ']', oos_get_block_name($bInfo->block_id, $languages[$i]['id']));
      }
