                 ...
                 if (MODULE_ORDER_TOTAL_GV_STATUS == 'true') {
                   ...
                   if (NEW_SIGNUP_GIFT_VOUCHER_AMOUNT > 0) {
                     ...
                     $email_text .= sprintf(EMAIL_GV_INCENTIVE_HEADER, $currencies->format(NEW_SIGNUP_GIFT_VOUCHER_AMOUNT)) . "\n\n" .
                                    ...
                                    EMAIL_GV_LINK . oos_catalog_link($aCatalogFilename['gv_redeem'], 'gv_no=' . $coupon_code) . 
                                    "\n\n";
                   }
  ...
  switch ($action) {
    ...
    default:
      ...
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        if ($login['status'] != '0') {
          $contents[] = array('align' => 'center', 'text' => oos_draw_login_form('login', $oosModules['admin'], $aCatalogFilename['login_admin'], 'action=login_admin','POST', 'target=_blank') . oos_draw_hidden_field('verif_key', $login['man_key']) . oos_draw_hidden_field('email_address', $cInfo->customers_email_address) . oos_image_swap_submits('login','login_off.gif', IMAGE_LOGIN) . '</form>');
        }
