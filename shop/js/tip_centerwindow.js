...
config. CenterWindow = false	// true or false - default is false, as we don't want this to be the default behaviour
...
ctrwnd.OnMouseMove = Ctrwnd_DoCenter;
...
function Ctrwnd_DoCenter()
{
	if(tt_aV[CENTERWINDOW])
	{
		...
		x = (tt_GetClientW() - tt_w) / 2 + dx;
		y = (tt_GetClientH() - tt_h) / 2 + dy;
		tt_SetTipPos(x, y);
		return true;
	}
	return false;
