...
/*
  ...
  /includes/classes/sepabanktransfer_validation.php
...
/* 5 ->   Diese SWIFT in lokaler DB nicht gefunden.                                       */
...
/* -------- Funktion zum externen Aufruf der Pruefung ---------- */
  function CheckAccount($dd_owner, $dd_iban, $dd_swift, $country_id) {
    // Erst einmal RICHTIG.
    $Result = 0;
    // Keinen Kontoinhaber uebergeben.
    if ($dd_owner == '' || strlen($dd_owner ) < 5) { return 1;}
    // Keine SWIFT uebergeben.
    if ($dd_swift == '') { return 8; }
    ...
    // SWIFT in lokaler DB suchen  -- im Fehlerfall return 5
      $Result = $this->UseLocalService($dd_iban, $dd_swift);
