<?php
/* ----------------------------------------------------------------------
   $Id: function_default.php 306 2013-04-15 01:01:23Z r23 $
  ...
  */
  function oos_get_path($current_category_id = '', $parent_id = '', $gparent_id = '') {
    ...
    if (oos_is_not_null($current_category_id)) {
      ...
      if ($cp_size == 0) {
        $category_new = $current_category_id;
      } else {
        $category_new = '';
        ...
        if ($parent_id == $gparent_id) {
          for ($i=0; $i < ($cp_size - 1); $i++) {
            $category_new .= '_' . $aCategoryPath[$i];
          }
        } else {
          for ($i=0; $i < $cp_size; $i++) {
            $category_new .= '_' . $aCategoryPath[$i];
        ...
        $category_new .= '_' . $current_category_id;

        if (substr($category_new, 0, 1) == '_') {
          $category_new = substr($category_new, 1);
        }
      }
    } else {
      $category_new = implode('_', $aCategoryPath);
    }

    return 'category=' . $category_new;
  }
