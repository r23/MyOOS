...
$w3all_conf_pref['wp_w3all_phpbb_iframe_short_pages_yn'] = (isset($w3all_conf_pref['wp_w3all_phpbb_iframe_short_pages_yn']) && ! empty($w3all_conf_pref['wp_w3all_phpbb_iframe_short_pages_yn'])) ? trim($w3all_conf_pref['wp_w3all_phpbb_iframe_short_pages_yn']) : '';
$w3all_conf_pref['wp_w3all_phpbb_iframe_short_token_yn'] = (isset($w3all_conf_pref['wp_w3all_phpbb_iframe_short_token_yn']) && ! empty($w3all_conf_pref['wp_w3all_phpbb_iframe_short_token_yn'])) ? trim($w3all_conf_pref['wp_w3all_phpbb_iframe_short_token_yn']) : '';
...
<br /><br /><br />
<b>phpBB real URL:</b> do NOT add final slash and set https or http, the same as WordPress<br /><br />
...
<?php // echo __('<span style="color:red;text-decoration:underline;">Read Important:</span> this option enable or disable the email check (if it already exist in phpBB) before it is updated in WordPress. When the update happen on front-end users profile pages by external plugins so you want to be sure that the updated email in Wp (and so in phpBB for the user with same email) do not belong to some other user in phpBB. Set this option to NO. Consider then that if an email is found duplicated into phpBB when an email update occur in WP, the email will reset to the old one.<br />Note that users profile pages managed by front end plugins, require that this option is set to NO (enabled).<br />Leaving this option set to Yes, will require that users have been all paired using the check and users transfer processes. Note that while this option is set to Yes, no check is done in phpBB when an email is updated in WP.<br />If you wish to let users to update their email and register also in phpBB, it is required to install the <a href="https://www.axew3.com/w3/2022/02/phpbb-wordpress-integration-common-tasks-extension/" target="_blank">phpBB WordPress Integration Common Tasks extension</a> into phpBB, or you\'ll disable registrations and the <i>Edit account settings</i> module in phpBB via<br /><i>ACP -> System Tab -> Module management -> User Control panel</i><br />disable the <i>Edit account settings</i> module (where user can change email and update password in phpBB)<br /><br />This option do not affect the default WordPress profile page or WP admin pages<br />Setting this option to No: if an email update occur and a duplicated email is found into phpBB, the user email value reset automatically to the old one', 'wp-w3all-phpbb-integration'); 

echo __('<span style="color:red;text-decoration:underline;">Read Important:</span> this option enable or disable the email check (if it already exist in phpBB) before it is updated in WordPress. When the update is done on front-end users profile pages by external plugins and to be sure that the updated email in WordPress do not belong to some other user in phpBB. If an email is found duplicated into phpBB when the email update occur in WP, the email will reset to the old one.<br />Note that users profile pages managed by front end plugins (like Memberpress) require this option Enabled<br /><br />This option do not affect the default WordPress profile page or WP admin pages, so if not using plugins with front end users profile pages, this option can be disabled<br />If enabled: when an email update occur in WordPress and a duplicated email is found into phpBB, the user email value will reset automatically to the old one<br />Leaving this option enabled is safe in any case', 'wp-w3all-phpbb-integration'); ?>


<p><input type="radio" name="w3all_conf_pref[w3all_disable_ck_email_before_wp_update]" id="w3all_disable_ck_email_before_wp_update_1" value="1" <?php checked('1', $w3all_conf_pref['w3all_disable_ck_email_before_wp_update']); ?> /> <?php echo __('Disable', 'wp-w3all-phpbb-integration'); ?></p>
<p><input type="radio" name="w3all_conf_pref[w3all_disable_ck_email_before_wp_update]" id="w3all_disable_ck_email_before_wp_update_0" value="0" <?php checked('0', $w3all_conf_pref['w3all_disable_ck_email_before_wp_update']); ?> /> <?php echo __('Enable', 'wp-w3all-phpbb-integration'); ?></p>
<hr />
...
<?php echo __('If some forum require specific permissions to be viewed and the user do not belong to this specific allowed group as primary Group, posts/topics from these forums are not retrieved to be displayed into Last Topics Widgets. Note that only the default user Group is considered', 'wp-w3all-phpbb-integration'); ?>
...
<p><input id="" name="w3all_conf_pref[wp_w3all_phpbb_iframe_short_token_yn]" type="text" size="25" value="<?php echo esc_attr( $w3all_conf_pref['wp_w3all_phpbb_iframe_short_token_yn'] ); ?>"> <?php echo __('<i>security_token</i> only <i>A-Z a-z 0-9</i> allowed ( example: aBc12345678901SSeLaN2050 OR MYsecret )', 'wp-w3all-phpbb-integration'); ?></p>
<hr />
<?php echo __('<h3 style="color:#003333">Activate [w3allcustomiframe] shortcode</h3>Advanced. Read <b>How To use</b> here: <a target="_blank" href="https://www.axew3.com/w3/2019/12/w3allcustomiframe-shortcode-how-to/">[w3allcustomiframe] shortcode how to</a>.<br />Activating this option, the iframe js resizer library will be added/loaded into WordPress header<br />to allow the javascript resizer code work properly on each WordPress page this shorcode will be added', 'wp-w3all-phpbb-integration'); ?>
