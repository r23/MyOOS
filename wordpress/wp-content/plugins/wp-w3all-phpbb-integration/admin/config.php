...
$w3all_conf_pref['w3all_disable_ck_email_before_wp_update'] = isset($w3all_conf_pref['w3all_disable_ck_email_before_wp_update']) ? $w3all_conf_pref['w3all_disable_ck_email_before_wp_update'] : 1;
...
<?php
    if( !defined('W3PHPBBDBCONN') ){
     echo __('<h3><span style="color:#ff0000">Before to activate the integration by setting phpBB db connection values it is mandatory to<br /><br /> read the</span> <i>phpBB side</i> <span style="color:#ff0000"">setup section into the <a target="_blank" href="https://www.axew3.com/w3/wordpress-phpbb-integration-install/">Install Help Page</a></span></h3>', 'wp-w3all-phpbb-integration');
...
<p><input id="" name="w3all_conf_pref[wp_w3all_phpbb_iframe_short_token_yn]" type="text" size="25" value="<?php echo esc_attr( $w3all_conf_pref['wp_w3all_phpbb_iframe_short_token_yn'] ); ?>"> <?php echo __('<i>security_token</i> param - only <i>A-Z a-z 0-9</i> allowed, or, like if empty, the option will not affect ( example: aWBfCg098JAH09z834HSG OR MYsecret )', 'wp-w3all-phpbb-integration'); ?></p>
