...
<?php echo __('<span style="color:red;text-decoration:underline">Important:</span> this option, active by default, can be <strong>Disabled setting it to Yes (so will be Disabled the email check)</strong>. Set to Yes and disable if there are problems with the message <span style="color:red;">"Error: email exists into our forum"</span> that could come out in some page, and/or any other error that could come out when some plugin goes to perform some task (example: Woocommerce memberships renewal process, users deletion, but not only)<br />Note that setting to Yes this option, it is required that users have been all paired using the check and users transfer process. If you wish to let users to update their email and register also in phpBB, you may will install the <a href="https://www.axew3.com/w3/2022/02/phpbb-wordpress-integration-common-tasks-extension/" target="_blank">phpBB WordPress Integration Common Tasks extension</a> into phpBB, or you\'ll disable registrations and the <i>Edit account settings</i> module in phpBB via<br /><i>ACP -> System Tab -> Module management -> User Control panel</i><br />disable the <i>Edit account settings</i> module (where user can change email and update password in phpBB)<br /><br />This option do not affect the default WordPress user profile page. And it is not required to disable this option into default WordPress installations<br />But it depend on how you configure WP and phpBB to run as integrated, and (more than this) how external plugins manage profile updates and WP hooks<br />Test your installation before to set to Yes (so the email check will be Disabled)', 'wp-w3all-phpbb-integration'); ?>
