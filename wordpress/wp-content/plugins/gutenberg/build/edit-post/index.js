...
(0,external_wp_data_namespaceObject.withSelect)((select, block) => {
  ...
  const firstOfSameType = (0,external_lodash_namespaceObject.find)(blocks, ({
    name
  }) => block.name === name);
...
function* openGeneralSidebar(name) {
  yield external_wp_data_namespaceObject.controls.dispatch(store.name, 'enableComplementaryArea', store_store.name, name);
}
/**
...
function* closeGeneralSidebar() {
  yield external_wp_data_namespaceObject.controls.dispatch(store.name, 'disableComplementaryArea', store_store.name);
}
/**
...
function* setAvailableMetaBoxesPerLocation(metaBoxesPerLocation) {
  ...
  const postType = yield external_wp_data_namespaceObject.controls.select(external_wp_editor_namespaceObject.store.name, 'getCurrentPostType');

  if (window.postboxes.page !== postType) {
    window.postboxes.add_postbox_toggles(postType);
  }

  let wasSavingPost = yield external_wp_data_namespaceObject.controls.select(external_wp_editor_namespaceObject.store.name, 'isSavingPost');
  let wasAutosavingPost = yield external_wp_data_namespaceObject.controls.select(external_wp_editor_namespaceObject.store.name, 'isAutosavingPost'); // Meta boxes are initialized once at page load. It is not necessary to
  // account for updates on each state change.
  //
  // See: https://github.com/WordPress/WordPress/blob/5.1.1/wp-admin/includes/post.php#L2307-L2309

  const hasActiveMetaBoxes = yield external_wp_data_namespaceObject.controls.select(store_store.name, 'hasMetaBoxes'); // First remove any existing subscription in order to prevent multiple saves
  ...
  saveMetaboxUnsubscribe = (0,external_wp_data_namespaceObject.subscribe)(() => {
    const isSavingPost = (0,external_wp_data_namespaceObject.select)(external_wp_editor_namespaceObject.store.name).isSavingPost();
    const isAutosavingPost = (0,external_wp_data_namespaceObject.select)(external_wp_editor_namespaceObject.store.name).isAutosavingPost(); // Save metaboxes on save completion, except for autosaves that are not a post preview.
    ...
    if (shouldTriggerMetaboxesSave) {
      (0,external_wp_data_namespaceObject.dispatch)(store_store.name).requestMetaBoxUpdates();
    }
  });
}
/**
...
function* requestMetaBoxUpdates() {
  ...
  const post = yield external_wp_data_namespaceObject.controls.select(external_wp_editor_namespaceObject.store.name, 'getCurrentPost');
  ...
  const activeMetaBoxLocations = yield external_wp_data_namespaceObject.controls.select(store_store.name, 'getActiveMetaBoxLocations');
  ...
  try {
    ...
    yield external_wp_data_namespaceObject.controls.dispatch(store_store.name, 'metaBoxUpdatesSuccess');
  } catch {
    yield external_wp_data_namespaceObject.controls.dispatch(store_store.name, 'metaBoxUpdatesFailure');
  }
}
/**
...
function* __unstableSwitchToTemplateMode(newTemplate = false) {
  ...
  const isWelcomeGuideActive = yield external_wp_data_namespaceObject.controls.select(store_store.name, 'isFeatureActive', 'welcomeGuideTemplate');
...
const isPluginItemPinned = (0,external_wp_data_namespaceObject.createRegistrySelector)(select => pluginName => {
...
function isMetaBoxLocationVisible(state, location) {
  return isMetaBoxLocationActive(state, location) && (0,external_lodash_namespaceObject.some)(getMetaBoxesPerLocation(state, location), ({
    id
  }) => {
...
function MetaBoxesSection({
  ...
  const thirdPartyMetaBoxes = (0,external_lodash_namespaceObject.filter)(metaBoxes, ({
    id
  }) => id !== 'postcustom');
  ...
  }), (0,external_lodash_namespaceObject.map)(thirdPartyMetaBoxes, ({
    id,
    title
...
function PreferencesModal() {
  ...
  const sections = (0,external_wp_element_namespaceObject.useMemo)(() => [{
    ...
    }, (0,external_wp_element_namespaceObject.createElement)(meta_boxes_section, null)))
  ...
  const {
    tabs,
    sectionsContentMap
  } = (0,external_wp_element_namespaceObject.useMemo)(() => sections.reduce((accumulator, {
    name,
    tabLabel: title,
    content
  }) => {
...
function FeatureToggle({
  ...
  return (0,external_wp_element_namespaceObject.createElement)(external_wp_components_namespaceObject.MenuItem, {
    ...
    isSelected: isActive,
    onClick: (0,external_lodash_namespaceObject.flow)(onToggle, speakMessage),
    role: "menuitemcheckbox",
    info: info,
    shortcut: shortcut
  }, label);
...
/* harmony default export */ var feature_toggle = ((0,external_wp_compose_namespaceObject.compose)([(0,external_wp_data_namespaceObject.withSelect)((select, {
  feature
}) => ({
  isActive: select(store_store).isFeatureActive(feature)
})), (0,external_wp_data_namespaceObject.withDispatch)((dispatch, ownProps) => ({
  onToggle() {
    dispatch(store_store).toggleFeature(ownProps.feature);
  }
...
function WritingMenu() {
  ...
  }, (0,external_wp_element_namespaceObject.createElement)(feature_toggle, {
...
function PostSchedule() {
  ...
  return (0,external_wp_element_namespaceObject.createElement)(external_wp_editor_namespaceObject.PostScheduleCheck, null, (0,external_wp_element_namespaceObject.createElement)(external_wp_components_namespaceObject.PanelRow, {
    ...
    renderToggle: ({
      onToggle,
      isOpen
    }) => (0,external_wp_element_namespaceObject.createElement)(external_wp_element_namespaceObject.Fragment, null, (0,external_wp_element_namespaceObject.createElement)(external_wp_components_namespaceObject.Button, {
...
function MetaBoxes({
  ...
  return (0,external_wp_element_namespaceObject.createElement)(external_wp_element_namespaceObject.Fragment, null, (0,external_lodash_namespaceObject.map)(metaBoxes, ({
    id
...
/* harmony default export */ var meta_boxes = ((0,external_wp_data_namespaceObject.withSelect)((select, {
  location
}) => {
  const {
...
const PluginDocumentSettingPanel = (0,external_wp_compose_namespaceObject.compose)((0,external_wp_plugins_namespaceObject.withPluginContext)((context, ownProps) => {
  if (undefined === ownProps.name) {
    typeof process !== "undefined" && process.env && "production" !== "production" ? external_wp_warning_default()('PluginDocumentSettingPanel requires a name property.') : void 0;
  }
...
function PostTemplateActions() {
  ...
  }, (0,external_wp_i18n_namespaceObject.__)('New'))), isModalOpen && (0,external_wp_element_namespaceObject.createElement)(external_wp_components_namespaceObject.Modal, {
...
const SettingsSidebar = () => {
  ...
  }, !isTemplateMode && sidebarName === 'edit-post/document' && (0,external_wp_element_namespaceObject.createElement)(external_wp_element_namespaceObject.Fragment, null, (0,external_wp_element_namespaceObject.createElement)(post_status, null), (0,external_wp_element_namespaceObject.createElement)(template, null), (0,external_wp_element_namespaceObject.createElement)(plugin_document_setting_panel.Slot, null), (0,external_wp_element_namespaceObject.createElement)(last_revision, null), (0,external_wp_element_namespaceObject.createElement)(post_link, null), (0,external_wp_element_namespaceObject.createElement)(post_taxonomies, null), (0,external_wp_element_namespaceObject.createElement)(featured_image, null), (0,external_wp_element_namespaceObject.createElement)(post_excerpt, null), (0,external_wp_element_namespaceObject.createElement)(discussion_panel, null), (0,external_wp_element_namespaceObject.createElement)(page_attributes, null), (0,external_wp_element_namespaceObject.createElement)(meta_boxes, {
    location: "side"
...
function Layout({
  ...
  const {
    ...
    return {
      ...
      hasActiveMetaboxes: select(store_store).hasMetaBoxes(),
      previousShortcut: select(external_wp_keyboardShortcuts_namespaceObject.store).getAllShortcutRawKeyCombinations('core/edit-post/previous-region'),
      nextShortcut: select(external_wp_keyboardShortcuts_namespaceObject.store).getAllShortcutRawKeyCombinations('core/edit-post/next-region'),
      showIconLabels: select(store_store).isFeatureActive('showIconLabels'),
      hasReducedUI: select(store_store).isFeatureActive('reducedUI'),
  ...
  return (0,external_wp_element_namespaceObject.createElement)(external_wp_element_namespaceObject.Fragment, null, (0,external_wp_element_namespaceObject.createElement)(fullscreen_mode, {
    ...
    }, (0,external_wp_element_namespaceObject.createElement)(meta_boxes, {
      location: "normal"
    }), (0,external_wp_element_namespaceObject.createElement)(meta_boxes, {
      location: "advanced"
...
function Editor({
  ...
  const editorSettings = (0,external_wp_element_namespaceObject.useMemo)(() => {
    const result = { ...(0,external_lodash_namespaceObject.omit)(settings, ['styles']),
