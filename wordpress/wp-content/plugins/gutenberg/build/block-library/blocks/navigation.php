 ...
 */
function gutenberg_gutenberg_get_menu_items_at_location( $location ) {
	if ( empty( $location ) ) {
		return;
	}
 ...
 */
function gutenberg_gutenberg_sort_menu_items_by_parent_id( $menu_items ) {
 ...
 */
function gutenberg_gutenberg_parse_blocks_from_menu_items( $menu_items, $menu_items_by_parent_id ) {
	if ( empty( $menu_items ) ) {
		return array();
	}
	...
	foreach ( $menu_items as $menu_item ) {
		...
		$block['innerBlocks'] = gutenberg_gutenberg_parse_blocks_from_menu_items(
...
function gutenberg_render_block_core_navigation( $attributes, $content, $block ) {
	...
	if ( empty( $inner_blocks ) && array_key_exists( '__unstableLocation', $attributes ) ) {
		$menu_items = gutenberg_gutenberg_get_menu_items_at_location( $attributes['__unstableLocation'] );
		if ( empty( $menu_items ) ) {
			return '';
		}

		$menu_items_by_parent_id = gutenberg_gutenberg_sort_menu_items_by_parent_id( $menu_items );
		$parsed_blocks           = gutenberg_gutenberg_parse_blocks_from_menu_items( $menu_items_by_parent_id[0], $menu_items_by_parent_id );
