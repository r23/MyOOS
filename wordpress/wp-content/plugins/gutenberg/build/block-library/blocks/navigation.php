...
function gutenberg_render_block_core_navigation( $attributes, $content, $block ) {
	...
	if ( $has_submenus && $should_load_view_script ) {
		...
		$inner_blocks_html = gutenberg_gutenberg_block_core_navigation_add_directives_to_submenu( $w, $attributes );
	}
 ...
 */
function gutenberg_gutenberg_block_core_navigation_add_directives_to_submenu( $w, $block_attributes ) {
	while ( $w->next_tag(
		array(
			'tag_name'   => 'LI',
			'class_name' => 'has-child',
		...
		gutenberg_gutenberg_block_core_navigation_add_directives_to_submenu( $w, $block_attributes );
	}
