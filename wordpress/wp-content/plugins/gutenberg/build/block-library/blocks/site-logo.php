 ...
 */
function gutenberg__sync_custom_logo_to_site_logo( $old_value, $value ) {
...
/**
 * Hooks `gutenberg__sync_custom_logo_to_site_logo` in `update_option_theme_mods_$theme`.
 *
 * Runs on `setup_theme` to account for dynamically-switched themes in the Customizer.
 */
function gutenberg_gutenberg__sync_custom_logo_to_site_logo_on_setup_theme() {
	$theme = get_option( 'stylesheet' );
	add_action( "update_option_theme_mods_$theme", 'gutenberg__sync_custom_logo_to_site_logo', 10, 2 );
}
add_action( 'setup_theme', 'gutenberg_gutenberg__sync_custom_logo_to_site_logo_on_setup_theme', 11 );
 ...
 * @return void
 */
function gutenberg__sync_site_logo_to_custom_logo( $old_value, $value ) {
	// Delete the option when the custom logo does not exist or was removed.
	// This step ensures the option stays in sync.
	if ( empty( $value ) ) {
		remove_theme_mod( 'custom_logo' );
	} else {
		remove_filter( 'pre_set_theme_mod_custom_logo', 'gutenberg__sync_custom_logo_to_site_logo' );
