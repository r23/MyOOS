 ...
 * @version   1.1.1
 * @author    Justin Tadlock <justin@justintadlock.com>, CampusPress <contact@campuspress.com>
...
class Breadcrumb_Trail {
	 ...
	 *     @type string    $list_class     The HTML tag to use for the list class.
	 *     @type string    $item_tag       The HTML tag to use for the item wrapper.
	 *     @type string    $item_class     The HTML tag to use for the item class.
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			//CP-TWEAKED-START
			$breadcrumb .= sprintf(
				'<%s class="%s" itemscope itemtype="http://schema.org/BreadcrumbList">',
			...
			foreach ( $this->items as $item ) {
				...
				$item_class = esc_attr( $this->args['item_class'] );

				if ( 1 === $item_position && 1 < $item_count ) {
					$item_class .= ' ' . $item_class . '-begin';

				} elseif ( $item_count === $item_position ) {
					$item_class .= ' ' . $item_class . '-end';
				...
				//CP-TWEAKED-START
				if ( $item_count === $item_position && $this->args['show_title'] ) {
	...
	protected function add_term_archive_items() {
		...
		if ( false === $done_post_type && $taxonomy->object_type && 1 === count( $taxonomy->object_type ) && post_type_exists( $taxonomy->object_type[0] ) ) {
