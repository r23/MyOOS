...
var kirkiPostMessage = {
	...
	toggleClass: {
		...
		fromOutput: function( output, value ) {
			...
			//CP-TWEAKED-START
			var optionValues = output.value;
			if( ! jQuery.isArray(optionValues) ) {
				var optionValues = [optionValues];
			}
			var classValues = output.class;
			if( ! jQuery.isArray(classValues) ) {
				var classValues = [classValues];
			}
			if ( jQuery.inArray( value, optionValues ) > -1 ) {
				_.each( classValues, function( classValue ) {
					if( ! jQuery( output.element ).hasClass( classValue ) ) {
						jQuery( output.element ).addClass( classValue );
					}
				});
			} else {
				_.each( classValues, function( classValue ) {
					jQuery( output.element ).removeClass( classValue );
				});
			}
			//CP-TWEAKED-END
		}
				...
				_.each( field.js_vars, function( output ) {
					...
					if ( 'css' === output.function ) {
						...
						//CP-START
							...
							//lets remove declarations from #kirki-inline-styles so default ones can kick in
							var internalStyles = jQuery( '#kirki-inline-styles' )[0];
							if( typeof internalStyles !== 'undefined' ) {
								jQuery.each( internalStyles.sheet.cssRules, function( index, cssRule ) {
									if( cssRule.selectorText == output.element ) {
										cssRule.style.removeProperty( output.property );
									} 
								});
							}
						}
