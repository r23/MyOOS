 ...
 * Author: Edward McIntyre - @twittem, WP Bootstrap, William Patton - @pattonwebz, CampusPress - @campuspresswp
 * Version: 4.1.0
...
if ( ! class_exists( 'CPSchool_WP_Bootstrap_Navwalker' ) ) {
	/**
	...
	class CPSchool_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			// Custom actions data
			$custom_action = get_post_meta( $item->ID, 'cps_custom_action', true );
			// Custom stylings data
			$custom_styling = get_post_meta( $item->ID, 'cps_custom_styling', true );
			...
			// Check if custom action opens custom dropdown
			if ( substr( $custom_action, 0, 9 ) == 'dropdown-' ) {
				$classes[]          = 'dropdown';
			...
			if ( $custom_styling == 'btn' ) {
				$atts['class'][] = 'has-styling';
				$atts['class'][] = 'btn';
				$atts['class'][] = 'btn-secondary';
			} elseif ( $custom_styling == 'btn-hl' ) {
				$atts['class'][] = 'has-styling';
				$atts['class'][] = 'btn';
				$atts['class'][] = 'btn-primary';
			}
			...
			if ( '' !== $linkmod_type ) {
				...
				if ( $item->object == 'wp_block' ) {
					...
					$megamenu_content = apply_filters( 'the_content', get_post_field( 'post_content', $item->object_id ) );
					$item_output     .= '<div id="' . esc_attr( $atts['id'] ) . '-dropdown" class="' . $class . '"><div class="dropdown-megamenu-menu-container container">' . $megamenu_content . '</div></div>';
				}

				// TODO - its not really done, it needs proper makeover
				if ( substr( $custom_action, 0, 9 ) == 'dropdown-' ) {
					global $translate_set;
					ob_start();

					//stupid workaround for just one bar as its tricky to support more
					if ( ! $translate_set ) {
						$translate_set = true
						?>
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$fallback_output .= '>';
				$fallback_output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="' . esc_attr__( 'Add a menu', 'cpschool' ) . '">' . esc_html__( 'Add a menu', 'cpschool' ) . '</a></li>';
				$fallback_output .= '</ul>';
				if ( $container ) {
					$fallback_output .= '</' . esc_attr( $container ) . '>';
				}
